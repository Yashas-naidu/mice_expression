...
import numpy as np
import pandas as pd
from mice_protein_expression.pipeline.prediction import PredictionPipeline
import math
    ...
    if request.method == 'POST':
        try:
            ...
            data = {
                ...
                'Genotype': int(request.form['Genotype']),
            ...
            if predict - math.floor(predict) < 0.5:
                approx_number = math.floor(predict)
            else:
                 approx_number = math.ceil(predict)

            return render_template('results.html', prediction=str(approx_number))
